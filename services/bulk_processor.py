...
class BulkProcessor:
    ...
    def process(
        ...
        df = df.copy()
        df = self._detect_duplicates(df, email_column)
        ...
        if unique_emails:
            company_domain = None
            column_results = self._process_batch(unique_emails, company_domain)
            for result in column_results:
                results_dict[result.normalized_email or result.original_email] = result

        result_df = self._map_results_back(df, results_dict, email_column, {})
        return result_df
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for future in as_completed(future_map):
                    ...
                    try:
                        ...
                        err_result = VerificationResult(original_email=batch[idx], notes=f"Error: {e}")
